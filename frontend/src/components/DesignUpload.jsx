...
export default function DesignUpload({ dressType, part, onUploadSuccess }) { // Accept onUploadSuccess prop
    ...
    const handleFileChange = (e) => {
        ...
        setMsg(""); // Clear messages on new file selection
        setMsgType("");
    };
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            if (!saveRes.ok) {
                ...
                document.querySelector("input[type=file]").value = ""; // Reset input value for re-uploading same file
            ...
            <div className="flex flex-col gap-3 w-full md:w-auto flex-grow">
                <label className="text-lg font-semibold text-gray-800 flex items-center">
                    <img
                        ...
                        onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/30x30/cccccc/333333?text=Err'; }}
                    />
                    Upload Design for <span className="text-indigo-700 ml-1">{dressType}</span> - <span className="text-purple-700 ml-1">{part}</span>
                </label>
                <input
                    type="file"
                    accept="image/*"
